...
from ttmrpp_nc.model.dual_encoder import DualEncoderModel
...
def load_ttmr_pp(model_types="last"):
    config = OmegaConf.load('../Data/ttmrpp_resnet_roberta.yaml')
    pretrained_object = torch.load('../Data/ttmrpp_resnet_roberta.pth', map_location='cpu')
...
def load_ttmr_probing(pretrain_dir, model_types="last"):
    ...
    model = ProbingLayer(
        audio_dim = 128,
        mlp_dim = config.mlp_dim,
        output_dim = 31, #number of annotation tags
